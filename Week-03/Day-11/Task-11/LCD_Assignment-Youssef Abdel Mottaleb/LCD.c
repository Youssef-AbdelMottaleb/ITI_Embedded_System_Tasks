...
{
	// Calculate the number of iterations required to complete one full wave
	u16 Local_u8NumIterations = (LCD_L2_BASE_ADDRESS - LCD_L1_BASE_ADDRESS) / MAX_AMPLITUDE;
	for (u16 Local_u8Count = 0; Local_u8Count < Local_u8NumIterations; Local_u8Count++)
	{
		// Calculate the vertical position using the sine function
		u8 yOffset = MAX_AMPLITUDE * sin(2 * M_PI * FREQUENCY * Local_u8Count / Local_u8NumIterations + PHASE_SHIFT);

		// Clear the display
		LCD_voidSendCommand(CLEAR_DISPLAY);

		// Set the LCD cursor to the calculated vertical position
		LCD_voidSetLocation(LCD_L1_BASE_ADDRESS + yOffset, 0);

		// Send the text to be displayed
		LCD_voidSendString(Copy_u8Text);

		_delay_ms(100);  // Delay for animation
	}
}
